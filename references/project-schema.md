...
artifacts_meta: {}                       # OAG 漂移检测：key -> { source_hash, rendered_at }
                                        # source_hash = 该交付物依赖数据切片(content-hash)，渲染时由
                                        # build_subproject.py / rerender_docs.py 写入；手工文档用
                                        # artifact_guard.py --stamp <key> 写入。artifact_guard.py 比对
                                        # 当前数据哈希与 source_hash，不一致即"数据漂移"(护栏违规)。
raid:
...
program: null                          # 仅 type=program：{ projects[], dependencies[], benefits[], sows[], contracts[], extracts[], sow_map[] }
                                   # sows[]: [{ sow, workstream, model, billing, fee }]  fee 必填(锁定值或 'TBC — 原因')，禁止（待定）
                                   # contracts[]: [{ code, type, scope, vendor, status, sow, fee }]
